...
const factors = [
  {
    ...
    icon: (
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
        <path d="M7 2v1M4.5 4.5L3.5 3.5M9.5 4.5l1-1M2 7h1M11 7h1M4.5 9.5C4.5 8.12 5.62 7 7 7s2.5 1.12 2.5 2.5" className="stroke-rose-800" strokeWidth="1.5" strokeLinecap="round" />
        <path d="M3.5 10.5C3.5 9.67 5.07 9 7 9s3.5.67 3.5 1.5" className="stroke-rose-800" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
        <rect x="1.5" y="4" width="11" height="7" rx="1.5" className="stroke-amber-800" strokeWidth="1.5" />
        <path d="M4.5 4V3a1 1 0 0 1 1-1h3a1 1 0 0 1 1 1v1" className="stroke-amber-800" strokeWidth="1.5" strokeLinecap="round" />
  ...
  },
  {
    ...
    icon: (
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
        <path d="M2 11L6 3l2 4 2-2 2 6" className="stroke-sky-700" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
        <rect x="1.5" y="5" width="11" height="7.5" rx="1" className="stroke-emerald-800" strokeWidth="1.5" />
        <path d="M4.5 5V3.5a2.5 2.5 0 0 1 5 0V5" className="stroke-emerald-800" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
        <circle cx="7" cy="7" r="5.5" className="stroke-slate-600" strokeWidth="1.5" />
        <path d="M7 4.5v3l2 1" className="stroke-slate-600" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
    ),
  },
]
...
export default function FactorRisksCard({ onSelectFactor }: FactorRisksCardProps) {
  const [expanded, setExpanded] = useState(false)
  const visibleFactors = expanded ? factors : factors.slice(0, 3)

  return (
    <Card variant="default" padding="none" className="flex flex-col bg-white overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 sm:px-5 py-4 border-b border-slate-200/80 bg-slate-50/50">
        <div className="flex items-center gap-2">
          <h3 className="font-bold text-sm text-slate-900">Ringkasan Risiko Utama</h3>
          <span className="cursor-help text-slate-400 hover:text-slate-600 transition-colors" title="Ringkasan faktor risiko utama lokasi properti">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
              <circle cx="7" cy="7" r="6" className="stroke-current" strokeWidth="1.5" />
              <path d="M7 6.5v4M7 4.5v.5" className="stroke-current" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </span>
        </div>
        <span className="text-xs text-slate-500 font-semibold tabular-nums">{factors.length} faktor dipantau</span>
      </div>
          ...
          >
            ...
            <div className="flex items-center gap-2.5 min-w-0 flex-1 sm:flex-none sm:w-52 shrink-0">
              ...
              <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                <span className="font-bold text-sm text-slate-900 whitespace-nowrap">{f.name}</span>
                {f.tag && (
                  <Badge variant="danger" size="sm" className="shrink-0 text-[10px]">
                    {f.tag}
                  </Badge>
                )}
              </div>
            ...
            <div className="shrink-0 text-right tabular-nums ml-auto sm:ml-0">
              {f.score !== null ? (
                <span className="text-xs font-bold whitespace-nowrap">
                  <span className="text-[#0F2B38] font-extrabold text-sm">{f.score}</span>
            ...
            <div className="shrink-0 flex items-center justify-end gap-1.5 min-w-[90px] sm:min-w-[110px]">
              <Badge variant={f.badgeVariant} pulse size="sm" className="shrink-0 text-[10px] sm:text-[11px]">
                {f.status}
              </Badge>
              <svg
                width="14"
      ...
      <div className="p-3 bg-slate-50/40 border-t border-slate-100 flex items-center justify-center">
        ...
        >
          <span>{expanded ? 'Sembunyikan faktor' : 'Tampilkan semua faktor'}</span>
          <svg
            width="14"
            height="14"
            viewBox="0 0 14 14"
