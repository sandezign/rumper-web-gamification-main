...
const SNAP_HEIGHTS: Record<SnapPoint, number> = {
  ...
  half: 0.5, // 50% fraction of window.innerHeight
  full: 0.9, // 90% fraction of window.innerHeight
}
...
export default function MobileBottomSheet({
  ...
  const currentHeight =
    dragging && dragHeight !== null ? dragHeight : snapPx(snap)
  ...
  const onTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      const next = Math.max(
        SNAP_HEIGHTS.peek,
        Math.min(snapPx("full"), startHeight.current + dy),
      )
      setDragHeight(next)
    },
    [dragging],
