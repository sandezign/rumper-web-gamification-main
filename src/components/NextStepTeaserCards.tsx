    ...
    <div className="flex flex-col gap-3">
      {steps.map((step) => (
        <div
          ...
          style={{ borderRadius: 20, border: '1px solid #E4EAED', boxShadow: '0 2px 8px rgba(15,42,51,0.05)' }}
        >
          {/* Lock hint row */}
          <div className="flex items-center gap-1.5 mb-2">
            <Lock size={11} color="#94A3B8" />
            <span className="text-xs" style={{ color: '#94A3B8' }}>{step.hint}</span>
          </div>

          {/* Content row */}
          <div className="flex items-center justify-between gap-3">
            <div className="flex flex-col gap-1.5">
              <p className="text-xs font-medium" style={{ color: '#64748B' }}>{step.teaser}</p>
              <div className="flex items-center gap-2">
                <span
                  className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full"
                  style={{ backgroundColor: step.badgeBg, color: step.badgeColor }}
                >
                  <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: step.badgeDot }} />
                  {step.badgeLabel}
                </span>
                <span className="text-xs font-medium" style={{ color: '#3d4f5b' }}>{step.actionLabel}</span>
              </div>
            </div>
