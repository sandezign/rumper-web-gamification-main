...
export default function DesktopSidebar({
  ...
  const isScenarioStage = flowStage >= 2 && flowStage <= 4
  const scenarioIndex = isScenarioStage ? flowStage - 2 : 0

  // Phase status helpers
  const isPhase1Active = flowStage === 1
  const isPhase1Done = flowStage > 1

  const isPhase2Active = flowStage >= 2 && flowStage <= 4
  const isPhase2Done = flowStage > 4

  const isPhase3Active = flowStage === 5
      ...
      <div className="flex flex-col gap-6">
        ...
        <div>
          ...
          {isPhase2Active && (
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold bg-[#003D4F] text-[#00ED64] border border-[#003D4F]">
              ...
              <span>Babak 2 · Skenario {scenarioIndex + 1} dari 3</span>
            </div>
          )}
          {isPhase3Active && (
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold bg-[#003D4F] text-[#00ED64] border border-[#003D4F]">
              <Clock size={14} className="shrink-0 text-[#00ED64]" />
              <span>Babak 3 · Langkah {parameterStep} dari 4</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          <h1 className="text-2xl lg:text-3xl font-extrabold text-white tracking-tight leading-tight">
            {isPhase1Active && 'Tentukan Batasan & Profil Anda'}
            {isPhase2Active && 'Pilih Prioritas Hunian Anda'}
            {isPhase3Active && 'Lengkapi Detail Profil Anda'}
          </h1>
          <p className="text-xs lg:text-sm text-[#A8B3BC] leading-relaxed">
            {isPhase1Active &&
              'Petakan risiko banjir dan waktu komuter riil sebelum survei ke lokasi.'}
            {isPhase2Active &&
              'Bandingkan 3 situasi nyata untuk menentukan apa yang paling penting bagi Anda.'}
            {isPhase3Active &&
              'Isi anggaran, lokasi rutin, dan koridor hunian incaran Anda.'}
          </p>
        </div>
        ...
        {isPhase2Active ? (
          <div className="bg-[#003D4F]/60 rounded-2xl p-4 border border-[#003D4F] space-y-2">
            <div className="flex items-center justify-between text-xs text-[#00ED64] font-bold">
              ...
              <span className="text-[11px] text-[#A8B3BC] font-medium">Uji Trade-off</span>
            </div>
          ...
          <div className="bg-[#003D4F]/60 rounded-2xl p-4 border border-[#003D4F] flex items-start gap-3">
            ...
            <p className="text-xs text-[#E1E5E8] leading-snug font-medium">
              Hanya konteks aktivitas harian Anda, tanpa data pribadi sensitif.
            </p>
          </div>
        )}
        ...
        <div className="mt-2 flex flex-col gap-0 relative">
          ...
          <div className="flex items-start gap-3.5 relative pb-5">
            ...
            >
              {isPhase1Done ? <Check size={16} className="stroke-[3]" /> : <Target size={16} />}
            </button>
            <div
              onClick={() => isPhase1Done && onStageClick && onStageClick(1)}
              className={`pt-1 flex flex-col ${isPhase1Done ? 'cursor-pointer' : ''}`}
            >
              <span
                className={`text-xs font-bold transition-colors ${
                  isPhase1Active ? 'text-white' : isPhase1Done ? 'text-[#E1E5E8]' : 'text-[#A8B3BC]'
                }`}
              >
                1. Kendala Utama
              </span>
              <span className="text-[11px] text-[#7C8C9A] font-medium">
                {isPhase1Done ? 'Kendala tercatat' : 'Tantangan pencarian'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3.5 relative pb-5">
            ...
            <button
              type="button"
              onClick={() => onStageClick && isPhase1Done && onStageClick(2)}
              className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 z-10 transition-all duration-300 ${
                isPhase2Active
            ...
            >
              {isPhase2Done ? <Check size={16} className="stroke-[3]" /> : <Sparkles size={16} />}
            </button>
            <div className="pt-1 flex flex-col">
              <span
                className={`text-xs font-bold transition-colors ${
                  isPhase2Active ? 'text-white' : isPhase2Done ? 'text-[#E1E5E8]' : 'text-[#A8B3BC]'
                }`}
              >
                2. Uji Prioritas Hunian
              </span>
              <span className="text-[11px] text-[#7C8C9A] font-medium">
                {isPhase2Active
                ...
                <div className="flex items-center gap-1.5 mt-2">
                  {[0, 1, 2].map((idx) => (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => onStageClick && onStageClick(idx + 2)}
          ...
          <div className="flex items-start gap-3.5 relative">
            ...
            <div className="pt-1 flex flex-col flex-1">
              ...
              >
                3. Detail Profil & Budget
              </span>
              <span className="text-[11px] text-[#7C8C9A] font-medium">
                {isPhase3Active ? `Langkah ${parameterStep} dari 4` : '4 parameter pencarian'}
              </span>
                  ...
                  {PARAMETER_STEPS.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isParamCompleted && <Check size={12} className="shrink-0 stroke-[3]" />}
                      </button>
                    )
                  })}
                </div>
