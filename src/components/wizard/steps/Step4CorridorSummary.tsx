...
export default function Step4CorridorSummary({ formData, onToggleCorridor }: Step4Props) {
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div>
        ...
        <h2 className="text-2xl md:text-3xl font-extrabold text-[#001E2B] tracking-tight">
          Koridor Wilayah Incaran & Ringkasan
        </h2>
        <p className="text-sm md:text-base text-[#5C6C7A] mt-1">
          Pilih area Jabodetabek yang sedang Anda lirik dan tinjau ringkasan profil pencarian Anda.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {CORRIDOR_OPTIONS.map((corridor) => {
            ...
            return (
              ...
              >
                <span className="text-xs md:text-sm leading-snug">{corridor}</span>
      ...
      <div className="bg-[#001E2B] rounded-3xl p-6 md:p-7 text-white shadow-xl space-y-5 border border-[#003D4F]">
        <div className="flex items-center justify-between border-b border-[#003D4F] pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-sm md:text-base font-bold text-white tracking-wide">
              Ringkasan Profil Pencarian Anda
            </h3>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs md:text-sm">
          <div>
            <span className="text-[#A8B3BC] font-medium block">Rumah Tangga & Kerja:</span>
            <span className="font-bold text-white mt-0.5 block">
              {householdLabelMap[formData.householdType]} · {workLabelMap[formData.workPattern]}
            </span>
          </div>

          <div>
            <span className="text-[#A8B3BC] font-medium block">Rentang Anggaran:</span>
            <span className="font-extrabold text-[#00ED64] mt-0.5 block">
              {formatRupiah(formData.budgetMin)} — {formatRupiah(formData.budgetMax)}
            </span>
          </div>

          <div>
            <span className="text-[#A8B3BC] font-medium block">Titik Aktivitas Utama (Kantor):</span>
          ...
          <div>
            <span className="text-[#A8B3BC] font-medium block">Titik Aktivitas Kedua:</span>
            <span className="font-bold text-white mt-0.5 block truncate">
        ...
        {Object.keys(formData.scenarioResponses).length > 0 && (
          <div className="pt-3 border-t border-[#003D4F] space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {formData.scenarioResponses['flood-vs-aesthetic'] === 'reject' && (
