...
const CENTER: [number, number] = [-6.266, 106.99]
...
const ROUTES = {
  krl: {
    ...
    endLabel: "Stasiun Bekasi",
    endPos: [-6.225, 106.986] as [number, number],
  },
  tol: {
    label: "Tol (Mobil)",
    color: "#2563EB",
    ...
    endLabel: "Pintu Tol Bekasi Timur",
    endPos: [-6.278, 107.022] as [number, number],
  },
  arteri: {
    label: "Arteri (Motor)",
    color: "#EA580C",
    ...
    endLabel: "Jl. Ahmad Yani",
    endPos: [-6.238, 106.956] as [number, number],
  },
  sekolah: {
    label: "Rute Sekolah",
    color: "#7C3AED",
    ...
    endLabel: "Sekolah terdekat",
    endPos: [-6.261, 106.984] as [number, number],
  },
}
...
const WALKABILITY_POIS = [
  {
    pos: [-6.264, 106.987] as [number, number],
    Icon: Store,
    color: "#0891B2",
    label: "Indomaret Galaxy",
  },
  {
    pos: [-6.268, 106.993] as [number, number],
    Icon: UtensilsCrossed,
    color: "#D97706",
    label: "Warung Makan Bu Sari",
  },
  {
    pos: [-6.263, 106.994] as [number, number],
    Icon: Star,
    color: "#16A34A",
    label: "Masjid Al-Hidayah",
  },
  {
    pos: [-6.261, 106.984] as [number, number],
    Icon: GraduationCap,
    color: "#7C3AED",
    label: "SDN Grand Galaxy",
  },
...
const TAB_CONFIG: Record<string, {
  ...
  Ringkasan: {
    center: CENTER,
    zoom: 15,
    infoLabel: "Flood Zone Evidence",
    infoSub: "BNPB 2024 · 95m from property",
    infoColor: "#E12626",
  },
  "Faktor risiko": {
    center: [-6.263, 106.99],
    zoom: 14,
    infoLabel: "Zona Risiko Aktif",
    infoSub: "2 layer risiko terdeteksi",
    infoColor: "#EA580C",
  },
  Perjalanan: {
    center: [-6.255, 106.985],
    zoom: 12,
    infoLabel: "4 rute tersedia",
    infoSub: "KRL · Tol · Arteri · Sekolah",
    infoColor: "#2563EB",
  },
  Checklist: {
    center: CENTER,
    zoom: 15,
    infoLabel: "Radius Jalan Kaki 500m",
    infoSub: "4 fasilitas esensial terjangkau",
    infoColor: "#16A34A",
  },
  Fasilitas: {
    center: [-6.263, 106.99],
    zoom: 13,
    infoLabel: "7 fasilitas ditemukan",
    infoSub: "Sekolah · RS · Mall · Taman · Ibadah",
    infoColor: "#7C3AED",
  },
}
...
function makeLucideIcon(
  ...
  const svg = renderToStaticMarkup(
    <Icon width={iconSize} height={iconSize} color="white" strokeWidth={2} />,
  )
...
function makePropertyIcon() {
  ...
  const svg = renderToStaticMarkup(
    <Home width={14} height={14} color="white" strokeWidth={2.2} />,
  )
...
function ZoomControls({
  ...
  recenterTarget: { center: [number, number] zoom: number }
  bottomOffset?: number
  side?: "left" | "right"
  fullscreen?: boolean
}) {
  ...
  return (
    <div
      ...
      } ${zIndexClass} flex flex-col gap-1.5`}
      style={{ bottom }}
    >
      {[
        ...
        { label: "−", title: "Zoom out", fn: () => map.zoomOut() },
        {
          ...
          title: "Recenter",
          fn: () =>
            map.flyTo(recenterTarget.center, recenterTarget.zoom, {
              duration: 0.5,
            }),
        },
      ].map((ctrl, i) => (
...
export default function MapPanel({
  ...
  const [activeRoutes, setActiveRoutes] = useState<Set<RouteKey>>(
    new Set(["krl", "tol", "arteri", "sekolah"]),
  )
  const [internalFullscreen, setInternalFullscreen] = useState(
    fullscreenProp ?? false,
  )
      ...
      <>
        {(isRingkasan || isRisiko) &&
          ([
            ...
            { key: "poiMarkers", label: "POI" },
            { key: "radiusCircle", label: "Radius" },
          ] as { key: keyof MapLayerState label: string }[]).map(
            ({ key, label }) => (
              <button
                key={key}
                onClick={() => toggle(key)}
                ...
                style={{
                  backgroundColor: layers[key] ? "#e3fcef" : "#f4f7f6",
                  color: layers[key] ? "#00684a" : "#7c8c9a",
                  border: `1px solid ${layers[key] ? "#00ed64" : "#e1e5e8"}`,
                }}
              >
                <span
                  className="w-1.5 h-1.5 rounded-full"
                  style={{
                    backgroundColor: layers[key] ? "#00D972" : "#c1ccd6",
                  }}
                />
                {label}
              </button>
        ...
        {isPerjalanan &&
          (Object.entries(ROUTES) as [RouteKey, typeof ROUTES[RouteKey]][]).map(
            ([key, r]) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium transition-all whitespace-nowrap"
                  style={{
                    backgroundColor: on ? `${r.color}18` : "#f4f7f6",
                    color: on ? r.color : "#7c8c9a",
                    border: `1px solid ${on ? r.color : "#e1e5e8"}`,
                  }}
                >
                  <span
                    className="w-1.5 h-1.5 rounded-full"
                    style={{ backgroundColor: on ? r.color : "#c1ccd6" }}
                  />
                  {r.label}
                </button>
              )
        ...
        {isFasilitas &&
          [
            { label: "Sekolah", color: "#7C3AED" },
            { label: "Kesehatan", color: "#DC2626" },
            { label: "Belanja", color: "#0891B2" },
            { label: "Ibadah", color: "#D97706" },
          ].map(({ label, color }) => (
            <span
              key={label}
              className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap"
  ...
  return (
    <div
      className="bg-white flex flex-col overflow-hidden transition-all duration-300 relative"
      style={wrapperStyle}
    >
      ...
      <div className="flex-1 relative" style={{ zIndex: 0 }}>
        ...
        >
          ...
          {/* RINGKASAN */}
          {isRingkasan && layers.radiusCircle && (
            <Circle
              center={CENTER}
              radius={600}
              pathOptions={{
                ...
                weight: 2,
                dashArray: "6 5",
                fillColor: "rgba(92,92,226,0.06)",
                fillOpacity: 1,
              }}
            />
          )}
          {isRingkasan && layers.flood && (
            <Polygon
          ...
          {isRingkasan && layers.poiMarkers && (
            <>
              <Marker position={[-6.258, 106.993]} icon={alertIcon}>
                <Popup>Zona banjir terdeteksi</Popup>
              </Marker>
              <Marker position={[-6.261, 106.984]} icon={schoolIcon}>
                <Popup>Sekolah terdekat</Popup>
              </Marker>
              <Marker position={[-6.266, 106.997]} icon={shopIcon}>
                <Popup>Fasilitas belanja</Popup>
              </Marker>
              <Marker position={[-6.272, 106.985]} icon={infraIcon}>
                <Popup>Infrastruktur</Popup>
              </Marker>
            </>
          )}
          ...
          {isRisiko && (
            <>
              ...
              {layers.flood && (
                <Polygon
                  positions={LIQUEFACTION_POLYGON}
                  pathOptions={{
                    color: "#EA580C",
                    weight: 2,
                    dashArray: "5 4",
              ...
              {layers.poiMarkers && (
                <>
                  <Marker position={[-6.258, 106.993]} icon={alertIcon}>
                    <Popup>Zona banjir terdeteksi — Tinggi</Popup>
                  </Marker>
                  <Marker position={[-6.262, 106.999]} icon={zapIcon}>
                    <Popup>Risiko likuifaksi — Sedang</Popup>
                  </Marker>
                </>
              )}
            </>
          ...
          {isPerjalanan && (
            <>
              {(Object.entries(
                ROUTES,
              ) as [RouteKey, typeof ROUTES[RouteKey]][]).map(([key, r]) =>
                activeRoutes.has(key) ? (
                  <Polyline
                    key={key}
                    positions={r.coords}
                    pathOptions={{
              ...
              {(Object.entries(
                ...
                activeRoutes.has(key) ? (
                  <Marker
                    key={`end-${key}`}
                    position={r.endPos}
                    icon={makeRouteEndIcon(r.color, r.endLabel)}
                  >
                    <Popup>
                      {r.label}: {r.endLabel}
                    </Popup>
          ...
          {isChecklist && (
            <>
              ...
              />
              {WALKABILITY_POIS.map((poi, i) => (
                <Marker
                  key={i}
                  position={poi.pos}
                  icon={makeLucideIcon(poi.Icon, poi.color, 32, 14)}
                >
          ...
          {isFasilitas &&
            FACILITIES.filter(
              (f) => !facilityVisible || facilityVisible[f.category],
            ).map((f, i) => (
              <Marker
                key={i}
                position={f.pos}
                icon={makeLucideIcon(f.Icon, f.color, 38, 17)}
              >
                <Popup>
          ...
          <Marker position={CENTER} icon={propertyIcon}>
            <Popup>
              <strong>Grand Galaxy City</strong>
              <br />
              Lokasi properti
            </Popup>
          </Marker>
          ...
          {!hideFloatingControls && (
            <ZoomControls
              ...
              bottomOffset={
                fullscreen ? 16 : sheetInset > 0 ? sheetInset + 116 : 76
              }
      ...
      {!fullscreen && !hideFloatingControls && (
        <div
          ...
          style={
            hideInfoChip
              ? undefined
              : { bottom: sheetInset > 0 ? sheetInset + 56 : 12 }
          }
        >
          {/* Info chip */}
          {!hideInfoChip && (
            ...
            >
              <span
                ...
                style={{ backgroundColor: tabCfg.infoColor }}
              />
              <p
