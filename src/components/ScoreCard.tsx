...
export default function ScoreCard({
  ...
  return (
    <Card variant="default" padding="none" className="p-4 sm:p-6 flex flex-col gap-4 sm:gap-5 bg-white">
      {/* ── Section Header ── */}
      <SectionHeader
        stepNumber={1}
        stepLabel="TAHAP"
      ...
      <div className="bg-slate-50/80 p-4 sm:p-5 rounded-2xl border border-slate-200/70 flex items-center justify-between gap-3 shadow-xs">
        ...
        <div className="relative w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center shrink-0">
          <svg width="68" height="68" viewBox="0 0 72 72" aria-hidden="true" className="drop-shadow-2xs">
            <circle
              cx="36"
              cy="36"
              r={radius}
      ...
      <div className="pt-3 border-t border-slate-100 flex items-center gap-2 sm:gap-3 flex-wrap text-xs text-slate-500 font-medium">
        <Badge variant="neutral" pulse size="sm" className="text-[10px] sm:text-xs">
          Tingkat Data: Sedang
        </Badge>
        <span className="text-slate-300">•</span>
        <span className="text-slate-600 font-medium text-xs">
          Terverifikasi dari <strong className="font-bold text-[#0F2B38] tabular-nums">{verifiedSourcesCount}</strong> sumber data resmi
        </span>
      </div>
    </Card>
  )
